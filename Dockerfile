...
RUN mkdir -p /usr/src/icpac_geoportal
...
COPY . /usr/src/icpac_geoportal/
WORKDIR /usr/src/icpac_geoportal
...
RUN chmod +x /usr/src/icpac_geoportal/tasks.py \
    && chmod +x /usr/src/icpac_geoportal/entrypoint.sh
...
ENTRYPOINT /usr/src/icpac_geoportal/entrypoint.sh
